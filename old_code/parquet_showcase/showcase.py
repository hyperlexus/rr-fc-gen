...
try:
    c_lib = ctypes.CDLL(os.path.abspath('c_stuff/fc_gen.dll'))
...
def generate_and_write_chunk_task(task_info):
    ...
    file_name = os.path.join(chunk_dir, f"data_chunk_{file_index}.parquet")

    num_pids = end_pid - start_pid

    fc_buffer = np.empty(num_pids * 12, dtype=np.uint8)
    mask_buffer = np.empty(num_pids, dtype=np.uint64)

    c_lib.generate_chunk(
        start_pid,
        end_pid,
        fc_buffer.ctypes.data_as(ctypes.c_void_p),
        mask_buffer.ctypes.data_as(ctypes.POINTER(ctypes.c_uint64))
    )

    fc_bytes = fc_buffer.view('S12')
    fc_array = pa.array(fc_bytes).cast(pa.string())
    mask_array = pa.array(mask_buffer)

    table = pa.Table.from_arrays([fc_array, mask_array], names=['fc', 'bitmask'])
    pq.write_table(table, file_name)

    return file_index, num_pids
...
class FriendCodeApp(tk.Tk):
    ...
    def setup_ui(self):
        ...
        gen_frame = ttk.LabelFrame(main_frame, text="create and populate database (once, about 5.4GB. takes ~6min)",
                                   padding="15")
        ...
        self.example_fc_label = ttk.Label(caesar_frame, text="example match (NOT REAL FC): ????-????-????",
                                          font=('Consolas', 14, 'bold'), foreground="#5db0d7")
    ...
    def generate_codes_task(self):
        try:
            ...
            with multiprocessing.Pool(processes=NUM_PROCESSES) as self.process_pool:
                async_results = [self.process_pool.apply_async(generate_and_write_chunk_task, args=(task,)) for task in
                                 tasks]
                ...
                while completed_chunks < len(tasks):
                    ...
                    self.thread_queue.put(
                        {"type": "progress", "action": "generating parquet chunks", "current": completed_chunks,
                         "total": NUM_CHUNKS})
                    time.sleep(0.1)
            ...
            self.thread_queue.put({"type": "indeterminate_progress", "action": "sorting 1 billion rows into database (takes ~4.5 minutes)"})
            ...
            conn.execute("PRAGMA memory_limit='20GB'")

            conn.execute(
                f"CREATE TABLE fcs AS SELECT fc, bitmask FROM read_parquet('{CHUNK_DIR}/*.parquet') ORDER BY bitmask")
            conn.close()

            # cleanup
            for i in range(NUM_CHUNKS):
                chunk_path = os.path.join(CHUNK_DIR, f'data_chunk_{i}.parquet')
            ...
            with open(MATCHES_FILE, 'w') as outfile:
                ...
                while True:
                    ...
                    if use_python_regex and not conditions and rows_scanned % 5_000_000 == 0:
                        self.thread_queue.put(
                            {"type": "indeterminate_progress", "action": f"scanning, {rows_scanned:,} rows done."})
    ...
    def format_matches_task(self):
        try:
            ...
            self.thread_queue.put({"type": "stop_indeterminate"})
            self.thread_queue.put({"type": "operation_complete",
                                   "text": f"formatted {len(matches)} matches as txt to {FORMATTED_MATCHES_FILE}"})
        except Exception as e:
